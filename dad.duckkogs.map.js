function loadMap(file) {
    ...
    xmlhttp.onreadystatechange=function() {
        if (xmlhttp.readyState == 3)
            loadMapTextures(result, xmlhttp.responseText);
    }
    xmlhttp.send();
    return result;
}

function loadMapTextures(result, responseText) {
    ...
    checkTextures(result, game_map, ready_textures);
}

function checkTextures(result, game_map, ready_textures) {
    if (ready_textures.n < game_map.textures.length) {
        setTimeout(function() {
            checkTextures(result, game_map);
        }, 100);
    }
    else {
        var contexts = [];
